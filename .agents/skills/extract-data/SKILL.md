...
4. **An own-site found → ask the user; don't judge it yourself.** Best-effort fetch the candidate and note what you saw (loads normally / 404 / parked / hosting-error / empty shell), then call the **AskUserQuestion** tool — **showing the exact URL** and your observation — to let the user decide whether the site is **working (available and reachable)** or **broken**, and how to proceed. Three outcomes:
   ...
   - **Working — build anyway** — the site is live, but build the dossier regardless. Seed `shop.website` with the URL (it then feeds the official-website source in steps 3, 4, 5 and the branding theme in step 8) and proceed to step 2.
...
After the fan-out returns, use the committed headless-browser helper to render the **public, JS-rendered** pages a plain fetch can't read: the business's **Facebook/Instagram pages** (their real web presence — most targets are website-less) and any **JS-rendered directory** source that came back empty. This is a best-effort fetch of a **public** page — **no login, no anti-bot bypass**, the same posture as `curl` but with a JS engine: it reaches what a logged-out visitor sees (including the first page of public post images), but **not** session-walled content. It runs in the **main agent** only (the fan-out sub-agents have web tools, not a browser).

Run it for:

- **The Facebook page and the Instagram page**, whenever one is known (`social_links.facebook` / `social_links.instagram`) — **one `--follow-posts` run each** — to harvest the page's self-reported facts (name, address, hours, contact) from the rendered DOM **and** the first page of public post images (each post's `images[]` / `og:image`). This is the single render pass over the social pages; step 7 downloads the images it surfaces.
- **Any selected source flagged "JS-rendered"** in `data-extraction.md` (e.g. SFF Member Register) that returned empty — re-extract its facts from the rendered DOM.

**Do not render the business's own website.** Step 4's `Official website` and `branding:website` agents already cover it; the render assist's job here is the login-adjacent social pages and empty JS directories, not the site.

```sh
node .agents/skills/extract-data/scripts/render.mjs <facebook-or-instagram-url> --follow-posts   # FB/IG: facts + posts[]
node .agents/skills/extract-data/scripts/render.mjs <js-directory-url>                            # directory: facts
```

One-time setup (Chromium may already be cached): run `npm install && npx playwright install chromium` in `scripts/` — see `scripts/README.md`. The JSON carries rendered **facts** (`title`, `metas`, `jsonLd`, `bodyText`, `links`), a `branding` block, and — with `--follow-posts` — a **`posts[]`** array of public post images. Fold the **facts** into the merge (step 5) under the **same source**, tagged "(rendered)" (e.g. `Facebook (rendered)`); hand the **`posts[]`** images to step 7 for download. **Ignore the `branding` block** — 4b only renders pages that aren't the business's own site (Facebook/Instagram, JS directories), so its computed styles are the platform's chrome, not the shop's (whose visual identity comes from its logo — step 8 tier 2). If the helper prints `{"ok":false,…}` or exits non-zero, record that source as `blocked`/`error` and move on — never abort the run.
...
- **Rendered sources:** if the render assist (step 4b) re-extracted a source — a Facebook/Instagram page, or a JS-rendered directory — fold its **facts** in under that same source, tagged "(rendered)" (e.g. `Facebook (rendered)`). Discard the render's `branding` block (it is the platform's chrome, not the business's — step 8 does not use it).
...
- Up to **20 deduplicated photos** → `assets/photo-001.<ext>`, `photo-002.<ext>`, … (extension from the content-type or URL). Dedup is by image URL, so the same photo served as resized CDN variants can slip through — don't treat the 20 as guaranteed-distinct.
  ...
  - **Facebook post photos (from step 4b).** The step-4b `--follow-posts` run already harvested these — each post's `images[]` holds the `scontent…fbcdn.net` URLs embedded in the logged-out photo page (FB leaves `og:image` empty, but the photo URL is in the markup). Download them with the `curl` recipe below; take the **logo** from the Facebook Graph profile picture (above). Caveats: it's behind a wall so it's **fragile** (FB can close the leak), the page also embeds the cover + adjacent thumbnails so candidates need confirming (does it show THIS business?), and URLs are signed/expiring. Aggregators (Restaurant Guru) and Google stay good fallbacks.
  - **Instagram post photos (from step 4b).** The step-4b `--follow-posts` run returns a `posts[]` array — the first page (~12 most-recent) of **genuine venue images** read from each post's public `og:image` (attribute to Instagram). Download them with the `curl` recipe below. Caveats: carousels give the cover image only, reels give the thumbnail frame, the og:image URLs are time-signed and **expire** (download promptly), and older posts beyond the first page need auth (out of scope). The profile **avatar** is still available via the profile's own `og:image`.
...
1. **Website (highest authority)** — if the step-4 `branding:website` agent returned usable data, canonicalize from it: palette roles, fonts, `theme-color`, shape/depth/spacing, vibe. Basis = **extracted**. Most targets are website-less, so this tier is usually empty — fall through to logo-derived.
...
- The **Branding / Theme** section exists with a labeled **Basis**, valid `#rrggbb` palette values, and named `display`/`body` fonts. If a usable website was found, the basis should be **extracted** — an **inferred** basis despite a known site means the step-4 branding agent failed; flag it.
