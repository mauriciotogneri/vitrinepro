...
- **Location map** (only when a map is shown): always embed **Google Maps**, **directly visible on page load with no click-to-load button** — a plain `<iframe>` (`https://www.google.com/maps?q=<lat>,<lng>&z=16&hl=<fr|en>&output=embed`) inside a fixed `aspect-ratio` box. This is a deliberate exception to `best-practices.md`'s click-to-load embed guidance — never use OpenStreetMap or a press-to-reveal placeholder.
  - **Never let the map iframe overflow or overlap neighbouring content** (a recurring bug): an `<iframe>` is a replaced element with a ~300px intrinsic width, so as a grid/flex item it has `min-width:auto` and **refuses to shrink**, pushing past its column over adjacent content. Make the embed bulletproof: the iframe wrapper is `position:relative; overflow:hidden` with a fixed `aspect-ratio`; the iframe itself is `position:absolute; inset:0; width:100%; height:100%` (so it never contributes to layout sizing); and every map/card track in the row gets `min-width:0`. Verify at desktop **and** mid/tablet widths that nothing overflows.
