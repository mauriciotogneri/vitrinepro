...
The form is not done at the HTML fields — it must also **react** to success and failure. Copy `references/snippets/contact-form.js` and `references/snippets/contact-form.css` verbatim per `site-conventions.md` → **Contact & location**, editing only the snippet's `MESSAGES` block (and, on a bilingual site, its `fr` flag) and the CSS's theme tokens. A bare form with no JS feedback is an incomplete build, not a smaller/faster one.
...
- In the Contact section's two-column layout, when the info column is taller than the form (the common case), the form is **bottom-aligned** to it (`justify-content:flex-end` on the form's flex column per `site-conventions.md` → **Contact & location**) — the submit button's bottom lines up with the last info-column card's bottom, not floating at the top with dead space below it.
- The contact form reacts to success and failure per `site-conventions.md` → **Contact & location**: `references/snippets/contact-form.js` copied verbatim (only `MESSAGES`, and a bilingual site's `fr` flag, edited) and `references/snippets/contact-form.css` retargeted to the site's own tokens — not a bare form with no JS. The form carries `class="contact-form"` (the fixed hook the snippet queries for) and nothing else hand-authors the `.btn-label`/`.spinner`/`.form-status` markup the script injects itself. `node --check js/main.js` passes.
