/* La Casa Del Sandwich — shared behaviour. Vanilla, no framework, no inline handlers. */
(function () {
  'use strict';
  var doc = document;
  ...
  var isOpenNow = function () {
    ...
    var r = schedule[zurichNow.day];
    return r ? (zurichNow.mins >= r[0] && zurichNow.mins < r[1]) : false;
  };
  ['open-status', 'open-status-2'].forEach(function (id) {
    ...
    if (txt) { txt.textContent = open ? (badge.dataset.open || 'Ouvert') : (badge.dataset.closed || 'Fermé'); }
  });
  ...
  var spyLinks = [].slice.call(doc.querySelectorAll('.site-nav a[href^="#"]'));
  ...
  var form = doc.querySelector('.contact__form form');
  if (form) {
    ...
    form.addEventListener('submit', function (e) {
      ...
      fetch(form.action, {
        ...
        if (r.ok) {
          ...
          setStatus('error', 'Une erreur est survenue. Réessayez, ou appelez-nous au 022 555 36 66.');
        }
      }).catch(function () {
        setStatus('error', 'Une erreur est survenue. Réessayez, ou appelez-nous au 022 555 36 66.');
      }).finally(function () {
