/* Café Metis — shared behaviour. Vanilla, no framework, no inline handlers. */
(function () {
  'use strict';
  var doc = document;
  ...
  /* --- current weekday + minute in Europe/Zurich --- */
  var zurichNow = null;
  try {
  ...
  /* --- opening schedule (minutes from midnight; an end > 1440 spills into
         the next day, e.g. Sat 17:00–26:00 means closing at 02:00 Sunday) --- */
  var ORDER = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];
  var SCHED = {
    Mon: [[690, 900], [1020, 1380]],
    Tue: [[690, 900], [1020, 1440]],
    Wed: [[690, 900], [1020, 1500]],
    Thu: [[690, 900], [1020, 1560]],
    Fri: [[690, 900], [1020, 1560]],
    Sat: [[1020, 1560]],
    Sun: []
  };
  function prevDay(d) { return ORDER[(ORDER.indexOf(d) + 6) % 7]; }
  function isOpenNow(now) {
    var today = SCHED[now.day] || [];
    for (var i = 0; i < today.length; i++) {
      if (now.mins >= today[i][0] && now.mins < Math.min(today[i][1], 1440)) { return true; }
    }
    var py = SCHED[prevDay(now.day)] || []; /* a session from yesterday spilling past midnight */
    for (var j = 0; j < py.length; j++) {
      if (py[j][1] > 1440 && now.mins < (py[j][1] - 1440)) { return true; }
    }
    return false;
  }
  ...
  if (badge) {
    if (zurichNow) {
      var openNow = isOpenNow(zurichNow);
