/* =====================================================================
   VetCity — main.js
   Vanilla, end-of-body. Year · mobile nav · scrollspy · scroll reveals.
   Followed by two standalone IIFEs: open-status (Europe/Zurich) + lightbox.
   ===================================================================== */
(function () {
  'use strict';
  var doc = document;
  ...
  var navMap = {};
  [].forEach.call(doc.querySelectorAll('.nav-links a[href^="#"]'), function (a) {
    var id = a.getAttribute('href').slice(1);
    if (id) { navMap[id] = a; }
  });
  var sections = [].filter.call(doc.querySelectorAll('main section[id]'), function (s) {
    return navMap[s.id];
  });
    ...
    form.addEventListener('submit', function (e) {
      ...
      fetch(form.action, {
        ...
        if (r.ok) {
          ...
          setStatus('error', 'Une erreur est survenue. Réessayez, ou appelez-nous au +41 22 995 96 97.');
        }
      }).catch(function () {
        setStatus('error', 'Une erreur est survenue. Réessayez, ou appelez-nous au +41 22 995 96 97.');
      }).finally(function () {
...
/* ── Open / closed status chip ──────────────────────────────────────────────
   ...
   Canonical IIFE — only the HOURS map is per-site.
   ──────────────────────────────────────────────────────────────────────────── */
(function () {
  'use strict';
  var doc = document;

  /* VetCity — Lun–Sam 08:00–18:00 · Dimanche fermé (480 = 08:00, 1080 = 18:00) */
  var HOURS = {
    0: [],
    1: [[480, 1080]],
    2: [[480, 1080]],
