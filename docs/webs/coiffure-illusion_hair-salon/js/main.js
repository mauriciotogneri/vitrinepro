/* Coiffure Illusion — vanilla JS, no framework.
   Year · mobile nav · scroll reveals · scrollspy
   + live open/closed status chip (Europe/Zurich) + photo lightbox. */
(function () {
  'use strict';
  var doc = document;
    ...
    form.addEventListener('submit', function (e) {
      ...
      fetch(form.action, {
        ...
        if (r.ok) {
          ...
          setStatus('error', 'Une erreur est survenue. Réessayez, ou appelez-nous au 022 329 42 20.');
        }
      }).catch(function () {
        setStatus('error', 'Une erreur est survenue. Réessayez, ou appelez-nous au 022 329 42 20.');
      }).finally(function () {
...
/* ── Open / closed status chip ──────────────────────────────────────────────
   ...
   Verbatim IIFE — only the HOURS map is per-site. French words are fixed;
   only the HH:MM follows the visitor's locale (timeZone stays Europe/Zurich).
   ──────────────────────────────────────────────────────────────────────────── */
(function () {
  'use strict';
  var doc = document;

  /* Opening hours (confirmed): Mar–Ven 09:00–18:30 · Sam 09:00–13:30 · Dim & Lun fermé.
     Minutes-of-day intervals keyed by getDay() (0=Sun … 6=Sat). 09:00=540, 18:30=1110, 13:30=810. */
  var HOURS = {
    0: [],
    1: [],
    2: [[540, 1110]],
