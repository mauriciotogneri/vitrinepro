/* Café Le Bout de la Rue — shared behaviour. Vanilla, no framework, no inline handlers. */
(function () {
  'use strict';
  var doc = document;
  ...
  /* --- current weekday + minutes in Europe/Zurich --- */
  var zurichNow = null;
  try {
  ...
  /* --- open / closed (hours self-reported by the bar) ---
     Mon–Wed 17:00–01:00 · Thu–Fri 17:00–02:00 · Sat 18:00–02:00 · Sun closed.
     Closing times run past midnight, so a session can spill into the next day. */
  var ORDER = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
  var SCHED = {
    Mon: [1020, 1500], Tue: [1020, 1500], Wed: [1020, 1500],
    Thu: [1020, 1560], Fri: [1020, 1560],
    Sat: [1080, 1560], Sun: null
  };
  function isOpen(day, mins) {
    var s = SCHED[day];
    if (s && mins >= s[0] && mins < Math.min(s[1], 1440)) { return true; }
    var prev = ORDER[(ORDER.indexOf(day) + 6) % 7];
    var ps = SCHED[prev];
    if (ps && ps[1] > 1440 && mins < ps[1] - 1440) { return true; }
    return false;
  }
  ...
  if (badge) {
    if (zurichNow) {
      var openNow = isOpen(zurichNow.day, zurichNow.mins);
      badge.dataset.state = openNow ? 'open' : 'closed';
      var txt = badge.querySelector('.txt');
      if (txt) {
        txt.textContent = openNow
          ? (badge.dataset.open || 'Ouvert')
          : (badge.dataset.closed || 'Fermé');
      }
    } else {
  ...
  form.addEventListener('submit', function (e) {
    ...
    fetch(form.action, {
      ...
      if (r.ok) {
        ...
        setStatus('error', 'Une erreur est survenue. Réessayez, ou appelez-nous au +41 22 320 51 51.');
      }
    }).catch(function () {
      setStatus('error', 'Une erreur est survenue. Réessayez, ou appelez-nous au +41 22 320 51 51.');
    }).finally(function () {
