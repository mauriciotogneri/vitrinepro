...
(function () {
  ...
  /* --- photo lightbox: click / Enter / Space to enlarge any content image ---
     Hooks every <picture> (content photos only — logos, icons and the map are
     never wrapped in <picture>), reusing the already-loaded asset. */
  ...
  if (zoomables.length) {
    ...
    var openLb = function (img) {
      ...
      if (!lb) { buildLb(); }
      /* collapse the mobile nav if open, so it isn't stranded behind the modal */
    ...
    /* Enter activates on keydown; Space on keyup — matches native <button> key behaviour */
    doc.addEventListener('keydown', function (e) {
