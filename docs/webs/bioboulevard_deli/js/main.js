/* BioBoulevard — shared behaviour. Vanilla, no framework, no inline handlers. */
(function () {
  'use strict';
  var doc = document;
  ...
  /* --- stagger index for card reveals --- */
  doc.querySelectorAll('.cards .card').forEach(function (el, i) {
  ...
  /* --- open / closed badge — Mon–Fri 09:30–19:00, Sat 09:30–18:00, Sun closed --- */
  function isOpen(now) {
    if (!now || now.day === 'Sun') { return false; }
    if (now.day === 'Sat') { return now.mins >= 570 && now.mins < 1080; }
    return now.mins >= 570 && now.mins < 1140; /* Mon–Fri */
  }
  var badge = doc.getElementById('open-status');
  if (badge) {
    if (zurichNow) {
      var openNow = isOpen(zurichNow);
  ...
  form.addEventListener('submit', function (e) {
    ...
    fetch(form.action, {
      ...
      if (r.ok) {
        ...
        setStatus('error', 'Une erreur est survenue. Réessayez, ou appelez-nous au 022 321 69 58.');
      }
    }).catch(function () {
      setStatus('error', 'Une erreur est survenue. Réessayez, ou appelez-nous au 022 321 69 58.');
    }).finally(function () {
