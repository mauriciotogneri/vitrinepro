...
_General food-and-drink directories (TripAdvisor, Just-Tag, Restaurant Guru, resto-rang.ch) list bars, pubs and cafés too — see **Food**._
...
- **[resto-rang.ch](https://www.resto-rang.ch/restaurants-a-geneve.html)** — _(Restaurant, Coffee Shop, Bar/Pub)_ _Coverage (Geneva/CH):_ strong Geneva/Romandie — community directory (~150k Geneva visits/month), classified by budget, district, cuisine. _Data:_ name, address, phone, hours, cuisine, price/budget, menus, user ratings & reviews. _Access:_ Scrape-only (HTML); **not** reachable to a plain fetch (HTTP 403) — needs the no-login headless render (`render.mjs`); no API.
...
- **[Boucherie Suisse](https://boucheriesuisse.com/c/geneve-ge/)** — _(Butcher Shop)_ _Coverage (Geneva/CH):_ moderate — Swiss butcher-address directory; Geneva, Carouge, Bernex pages live. _Data:_ business name, street address, phone. _Access:_ Scrape-only (HTML); a plain fetch returns HTTP 403 — needs the no-login headless render (`render.mjs`). (Meta-description carries a stray "Province de Luxembourg" template artifact; the listings are correct Geneva.)

_Sub-type note: **Donut Shop**, **Ice Cream Shop** and **Coffee Shop** have no dedicated CH/Geneva platform — they surface only via generic listings (TripAdvisor, Too Good To Go). **Bakery** likewise has no specialist directory but is well covered by Too Good To Go._
...
- **[pressingsuisse.com](https://pressingsuisse.com/)** — _(Dry Cleaner)_ _Coverage (Geneva/CH):_ moderate — dedicated CH pressing/dry-cleaner directory with Geneva pages (geneve-ge, etc.); community-editable so completeness varies. _Data:_ name, address, services, user reviews. _Access:_ Scrape-only (HTML); a plain fetch returns HTTP 403 — needs the no-login headless render (`render.mjs`); no API.
- **[laveriesuisse.com](https://laveriesuisse.com/)** — _(Laundromat)_ _Coverage (Geneva/CH):_ moderate — dedicated CH laundromat/laverie directory with Geneva pages (Meyrin, Versoix, Geneva); community-editable. _Data:_ name, address, services, user reviews. _Access:_ Scrape-only (HTML); a plain fetch returns HTTP 403 — needs the no-login headless render (`render.mjs`); no API.
