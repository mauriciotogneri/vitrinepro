...
- **Bands are a solid colour — never a pattern.** Fill every section background with a flat theme tone; do **not** lay a repeating texture (polka dots, stripes, noise, a tiled image, or a gradient used as texture) over a section/band background — a pattern behind content reads as busy and undercuts legibility, including on deep feature bands. A contained decorative graphic device elsewhere — a small shape or dotted "plate" sitting *behind an image*, an inline motif — is fine; this rule governs the full-bleed **section background** only.
...
- A **Formspree** POST form `<form class="contact-form" action="https://formspree.io/f/{{FORMSPREE_ID}}" method="post">` collecting **name, email, and message only**, with labelled (`<label for>`), `autocomplete`-tokened, typed, `required` fields, and a single `<button type="submit">Envoyer le message</button>`. **Never** build a reservation/booking form or add its fields (date, time-slot, party size, appointment picker), even for restaurants or businesses that take bookings — a plain contact form only. The `{{FORMSPREE_ID}}` is a placeholder — **flag it** in the summary as must-replace for the form to work. The `class="contact-form"` is a **fixed, required class name** — not a styling choice — because the canonical success/failure script below finds the form by it; don't rename it.
- **Success / failure feedback is required, not optional** — a bare form that silently POSTs and redirects (or does nothing until Formspree's own page loads) is an incomplete build. **The JS is canonical — copy it verbatim** from `references/snippets/contact-form.js` (a standalone IIFE appended to `js/main.js` next to the open-status/lightbox IIFEs; it declares its own `doc`, depends on nothing else, and injects its own status/spinner markup at runtime — **do not hand-author `.btn-label` / `.spinner` / `.form-status` HTML**, the script builds it from the plain button described above). The **only per-site edit is the `MESSAGES` block** — the phone number or fallback contact channel in the error string, sourced from **however that site already displays it elsewhere on the page** (never reformatted from a raw `tel:` href by hand) — plus, on a **bilingual site** (an `/en` mirror sharing this same `js/main.js`), uncommenting the snippet's local `fr` flag and using its bilingual `MESSAGES` ternary instead. **CSS is themed per site** in that site's tokens, copied from `references/snippets/contact-form.css` and remapped per its header comment (spinner colour, transition tokens, an ok/success colour, an error colour — fall back to a literal accessible green/red if the palette has no clean equivalent). The mechanism itself — `checkValidity()` gate, `FormData` captured **before** disabling any field (disabled fields are silently excluded from `FormData` — a real, easy-to-reintroduce bug if reordered), full-field + button disable with `aria-busy` during the request, `Accept: application/json` so Formspree replies with JSON instead of redirecting, `form.reset()` + a success message on `r.ok`, an error message on a bad response **and** on network failure (`.catch`) — stays untouched.
- Alongside it, the real contact paths from the dossier: `tel:`, `mailto:`, and WhatsApp (`https://wa.me/<intl-number>`).
- **Address → clickable Google Maps link (always):** wherever the page shows the venue's street address — the contact "visit us / in store" block **and/or** the location/"find us" section beside the map — make the **address text itself the link** to Google Maps, never plain static text: `<a href="https://www.google.com/maps/dir/?api=1&destination=<lat>,<lng>" target="_blank" rel="noopener noreferrer">…address…</a>`, reusing the **same `<lat>,<lng>` as the map embed** (`dir/?api=1`, **not** a `?q=` "open in Maps" link). If the business has no map/coords, fall back to `destination=<URL-encoded "Street nº, postcode City">`. The linked address is the **only** directions affordance — **never add a separate "Voir l'itinéraire" / directions button** (or any standalone "open in Maps" CTA) beside it. Apply it even when the map is also embedded on the page.
- **Two-column layout (always):** put the **contact form on the right** and the **contact info on the left** (the native links — phone, WhatsApp, address / "visit in store" block). Keep the **DOM/source order matching this visual order** (info block first, then the form) so reading and focus order stay aligned — never reorder with CSS `order`/`grid-column`. On narrow screens they stack in that same order (info, then form).
  - **Bottom-align the form with the info column when they differ in height (recurring gap).** The info column (phone/WhatsApp/e-mail/address/social cards) is almost always taller than the form (3 fields + a button), so it sets the row height. Grid `align-items:stretch` (the default) is still correct — it keeps the two columns equal height — but that only stretches the **form's own box**; a plain `.form{display:flex;flex-direction:column}` packs its fields+button at the **top** of that taller box, leaving dead space below the submit button and the form looking adrift from the info column's last card. Add `justify-content:flex-end` to the form's flex column so its content sits flush at the **bottom** instead — the submit button's bottom then lines up with the last info-column card's bottom. This repositions the form only; never grow the fields/textarea/button to fill the extra height, and never apply this on the narrow single-column stack (there the two blocks aren't stretched against each other, so it's a no-op there anyway).
...
**The JS is canonical — copy it verbatim** from `references/snippets/open-status.js` (a standalone IIFE appended to `js/main.js` next to the lightbox IIFE; it declares its own `doc` and depends on nothing else). The **only per-site edit is the `HOURS` map** — minutes-of-day intervals keyed by `getDay()` (`0 = Sun … 6 = Sat`, e.g. `07:30` → `450`), filled from the dossier; the `Europe/Zurich` computation, the locale-formatted clock, and the French labels stay untouched. (The snippet assumes each day's hours fall within one calendar day; venues open **past midnight** need it extended — flag it if so.)
...
- When hours are known, render the live open/closed status chip in `Europe/Zurich` — a standalone IIFE copied verbatim from `references/snippets/open-status.js`, only its `HOURS` map per-site (see **Open/closed status chip**); with no hours, omit the IIFE and render nothing.
...
- Wire the **contact form**'s success/failure feedback (see **Contact & location**) — a standalone IIFE copied verbatim from `references/snippets/contact-form.js`, only its `MESSAGES` block edited per-site; it injects its own status/spinner markup, so no matching HTML is hand-authored.
